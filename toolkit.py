...
def get_chart(ticker, interval, timerange, finalData):
    
    if ticker in finalData["Ticker"].values:
        ...
        if  localData["Price"].dropna()[0] > 12: # If the chart starts with the price higher than the entry threshold -> skip this company
            ...
            try:
                ...
                finalData = finalData.append({"Ticker": ticker,
                             ...
                             "Buy Price": "$%.2f" % get_buy_price(localData, start_date_index),
...
def plot_chart(ticker, finalData):
    ...
    buy_price = get_buy_price(finalData["Price Data"].iloc[index], get_jump_date(finalData["Price Data"].iloc[index]))
    ...
    if target_date_index == None:
        ...
        for i in range(start_date_index, target_date_index):
            ...
            if str(localData["Price"][start_date_index+n]) == "nan":
                ...
                cum_rets += 100*(localData["Price"][start_date_index+n]-localData["Price"][start_date_index])/localData["Price"][start_date_index]
...
def get_current_returns(localData, start_date_index):
    ...
    if localData["Price"].iloc[start_date_index] > 0:
        ...
        rets = 100 * (localData["Price"].dropna().iloc[-1]-localData["Price"].iloc[start_date_index])/localData["Price"].iloc[start_date_index]
    else:
        
        average = localData["Price"].iloc[:start_date_index].mean()
        rets = 100 * (localData["Price"].dropna().iloc[-1]-average)/average
         
    return(rets)

def get_buy_price(localData, start_date_index):
    # If the price just before the price jump is unknown than we take the average price obsrved before this timepoint
    if start_date_index == None:
        return(None)
    else:
        if localData["Price"].iloc[start_date_index] > 0:
